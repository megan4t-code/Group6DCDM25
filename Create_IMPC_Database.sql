...
create table impc_gene (
    ...
    constraint impc_gene_pk primary key (mgi_accession_id)
)
...
create table impc_analysis (
   ...
   foreign key (mgi_accession_id) references impc_gene (mgi_accession_id),
...
create table impc_gene_disease (
    ...
    constraint impc_gene_disease_pk primary key (mgi_accession_id, disease_id),
    foreign key (mgi_accession_id) references impc_gene (mgi_accession_id),
...
create index idx_disease_name on impc_disease (disease_name)

create index idx_gene_symbol on impc_gene (gene_symbol)

create index idx_parameter_name on impc_parameter (parameter_name)

create index idx_analysis_columns on impc_analysis (mgi_accession_id, impc_parameter_orig_id)
...
-- Temporary tables are being created because we want to move columns from one table to another using MySQL commands instead of editing the data. 
-- For example, we want to replace  the parameter id column in the analysis table with the impc original id.
-- These tables are just to move around information we will then delete them.

create table raw_parameter_data (
...
-- here we created a table with the same structure and column names as the raw data so it can be imported.

insert into impc_parameter (
...
select
    ...
    parameter_name,
    description
from raw_parameter_data

-- here we inserted the data into the final impc_parameter table.
...
select
    raw_analysis_data.analysis_id,
    raw_analysis_data.gene_accession_id,
    raw_analysis_data.mouse_strain,
    raw_analysis_data.mouse_life_stage,
    impc_parameter.impc_parameter_orig_id,
    raw_analysis_data.pvalue
from raw_analysis_data
join impc_parameter
    on raw_analysis_data.parameter_id = impc_parameter.parameter_code
